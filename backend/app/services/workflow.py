...
def create_payment_link(db:Session,merchant_id:str,decision:AgentDecision)->RecoveryAction:
    ...
    if action.provider_reference:return action
    ...
    try:result=provider(db,merchant_id,policy).create_payment_link(order.amount_paise,action.id,f"Recover order {order.external_ref}",{"name":customer.name,"email":customer.email,"phone":customer.phone})
    ...
    action.execution_mode=result.mode;action.provider_reference=result.provider_id;action.provider_url=result.url;action.status="executed";action.execution_result={"provider_status":result.status};action.executed_at=utcnow()
