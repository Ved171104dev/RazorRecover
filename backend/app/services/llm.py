...
def narrate(question:str,facts:dict,fallback:str)->tuple[str,str]:
    ...
    try:
        payload={"model":model,"store":False,"max_output_tokens":260,"instructions":"You explain merchant revenue recovery evidence. Use only the supplied database facts. Do not invent, change, calculate, or add financial numbers. Do not authorize or execute actions. If facts are insufficient, say so.","input":f"Merchant question: {question}\\nAuthenticated database facts: {json.dumps(facts,separators=(',',':'))}"}
