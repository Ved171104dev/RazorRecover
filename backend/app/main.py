...
from app.services.seed import bootstrap_demo,create_merchant_account,seed_merchant
from app.services.workflow import create_payment_link,ensure_action,provider,verify_and_attribute
...
def signup(body:Signup,response:Response,request:Request,db:Session=Depends(db_session)):
    ...
    user,merchant=create_merchant_account(db,body.name,body.email,body.password,body.merchant_name);seed_merchant(db,merchant)
...
def dashboard(p:Principal=Depends(auth),db:Session=Depends(db_session)):
    ...
    policy=db.scalar(select(MerchantPolicy).where(MerchantPolicy.merchant_id==mid));connected=bool(os.getenv("RAZORPAY_KEY_ID","").startswith("rzp_test_") and os.getenv("RAZORPAY_KEY_SECRET")) and not policy.simulation_mode
    return {"mode":"LIVE TEST MODE — NO REAL MONEY" if connected else "DEMO SIMULATION MODE — NO REAL MONEY","metrics":{"revenue_at_risk_paise":risk,"recoverable_revenue_paise":expected,"recovered_revenue_paise":recovered,"recovery_rate":round(recovered/risk*100,1) if risk else 0,"incremental_revenue_paise":max(0,recovered),"ai_actions":ac,"successful_actions":successful,"blocked_actions":blocked,"pending_approvals":pending,"active_experiments":int(db.scalar(select(func.count()).select_from(Experiment).where(Experiment.merchant_id==mid,Experiment.status=="running")) or 0)},"events":[{"id":e.id,"stage":e.stage,"title":e.title,"detail":e.detail,"amount_paise":e.amount_paise,"created_at":e.created_at.isoformat()} for e in events],"charts":{"by_cause":[{"name":x,"value_paise":int(v)} for x,v in bycause],"recovery_series":[{"day":x,"recovered_paise":recovered if x=="Today" else 0} for x in ["-6d","-5d","-4d","-3d","-2d","Yesterday","Today"]]}}
...
def create_test_order(body:OrderCreate,p:Principal=Depends(mutation),db:Session=Depends(db_session)):
    ...
    try:r=provider(policy).create_order(o.amount_paise,o.currency,f"rr_{o.id[:24]}")
    except ProviderError as exc:raise HTTPException(502,str(exc))
    rec=RazorpayOrder(merchant_id=p.merchant_id,internal_order_id=o.id,razorpay_order_id=r.provider_id or f"simulated:{o.id}",amount_paise=o.amount_paise,currency=o.currency,status=r.status,raw_data=r.raw);db.add(rec);db.commit()
...
def sync_test_order(record_id:str,p:Principal=Depends(mutation),db:Session=Depends(db_session)):
    ...
    if rec.razorpay_order_id.startswith("simulated:"):return {"mode":"simulated","status":rec.status,"payments":[]}
    try:data=provider(policy).fetch_order(rec.razorpay_order_id);payments=provider(policy).list_order_payments(rec.razorpay_order_id)
    except ProviderError as exc:raise HTTPException(502,str(exc))
...
def demo(p:Principal=Depends(mutation),db:Session=Depends(db_session)):
    d=db.execute(select(AgentDecision).join(RiskEvent,AgentDecision.risk_event_id==RiskEvent.id).join(Payment,RiskEvent.payment_id==Payment.id).join(Order,Payment.order_id==Order.id).where(AgentDecision.merchant_id==p.merchant_id,AgentDecision.selected_action=="recovery_link").order_by(desc(Order.amount_paise))).scalars().first()
    if not d:raise HTTPException(404,"No eligible demo opportunity")
    a=ensure_action(db,p.merchant_id,d)
    if a.status=="awaiting_approval":
        ap=db.scalar(select(Approval).where(Approval.recovery_action_id==a.id));ap.status="approved";ap.reviewed_by_user_id=p.user_id;ap.reviewed_at=utcnow();a.status="approved";db.commit()
    a=create_payment_link(db,p.merchant_id,d)
    if a.execution_mode=="simulated" and a.verification_status!="verified":
        pay=db.get(Payment,a.payment_id);verify_and_attribute(db,a,None,pay.amount_paise,"deterministic_demo_simulation")
    return {"risk":risk_out(db,db.get(RiskEvent,d.risk_event_id)),"decision":decision_out(db,d),"action":action_out(a),"label":"SIMULATED" if a.execution_mode=="simulated" else "RAZORPAY TEST MODE — awaiting customer payment"}
...
def assistant(body:AssistantQuery,p:Principal=Depends(mutation),db:Session=Depends(db_session)):
    ...
    elif "strategy" in q or "experiment" in q:tools=["get_experiment_results","get_strategy_performance"];e=experiments(p,db)["items"][0];answer=f"{e['winner'] or 'No winner'} has the highest observed recovery rate. Sample sizes and confidence intervals are shown; statistical significance is not claimed."
...
class Settings(BaseModel):
    automatic_threshold_paise:int=Field(ge=0,le=100000000);approval_threshold_paise:int=Field(ge=0,le=100000000);blocked_threshold_paise:int=Field(ge=0,le=500000000);max_retries:int=Field(ge=0,le=10);minimum_confidence:float=Field(ge=0,le=1);cooldown_minutes:int=Field(ge=0,le=10080);allowed_actions:list[str];simulation_mode:bool
@app.get("/api/settings")
def get_settings(p:Principal=Depends(auth),db:Session=Depends(db_session)):
    s=db.scalar(select(MerchantPolicy).where(MerchantPolicy.merchant_id==p.merchant_id));c=db.scalar(select(RazorpayConnection).where(RazorpayConnection.merchant_id==p.merchant_id));kid=os.getenv("RAZORPAY_KEY_ID","");configured=bool(kid.startswith("rzp_test_") and os.getenv("RAZORPAY_KEY_SECRET"))
    return {**{x:getattr(s,x) for x in ["automatic_threshold_paise","approval_threshold_paise","blocked_threshold_paise","max_retries","minimum_confidence","cooldown_minutes","allowed_actions","simulation_mode"]},"razorpay":{"connected":configured,"key_id_masked":kid[:9]+"********" if configured else None,"webhook_status":c.webhook_status,"mode":"TEST MODE — NO REAL MONEY" if configured else "DEMO SIMULATION MODE"}}
@app.put("/api/settings")
def update_settings(body:Settings,p:Principal=Depends(mutation),db:Session=Depends(db_session)):
    if not body.simulation_mode and not (os.getenv("RAZORPAY_KEY_ID","").startswith("rzp_test_") and os.getenv("RAZORPAY_KEY_SECRET")):raise HTTPException(422,"Razorpay Test Mode credentials are required before disabling simulation")
