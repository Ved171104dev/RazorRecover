...
def source_out(db:Session,merchant_id:str)->dict:
    ...
    return {"razorpay":{"connected":bool(c and c.connection_status=="connected"),"mode":"TEST MODE — NO REAL MONEY" if c and c.connection_status=="connected" else "NOT CONNECTED","key_id_masked":c.key_id_masked if c else None,"webhook_status":c.webhook_status if c else "not_verified","webhook_url":f"{public}/api/webhooks/razorpay/{c.webhook_token}" if c else None,"last_verified_at":c.last_verified_at.isoformat() if c and c.last_verified_at else None,"last_sync_at":c.last_sync_at.isoformat() if c and c.last_sync_at else None,"sync_status":c.sync_status if c else "never","sync_error":c.sync_error if c else None,"imported_orders":c.imported_orders or 0 if c else 0,"imported_payments":c.imported_payments or 0 if c else 0},"imports":[ingestion_run_out(x) for x in runs],"upload":{"max_bytes":10000000,"max_rows":5000,"formats":["csv","tsv","xlsx","xls","json","pdf"],"required_columns":["external_id","order_id","customer_email","customer_name","amount_paise","status","method","failure_code"],"optional_columns":["currency","customer_phone"]}}
